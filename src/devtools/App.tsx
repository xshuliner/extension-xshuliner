...
export default function DevtoolsPanelApp() {
  ...
  const sendMessage = () => {
    if (
      ...
      browser.tabs
        .sendMessage(browser.devtools.inspectedWindow.tabId, {
  ...
  const evaluateCode = () => {
    if (
      ...
      browser.devtools.inspectedWindow
        .eval('console.log("Hello from DevTools!");')
        .then(result => {
          ...
          if (exceptionInfo && exceptionInfo.isException) {
            setMessages(prev => [
              ...prev,
              `Eval exception: ${exceptionInfo.value || exceptionInfo.description}`,
            ]);
          } else {
            setMessages(prev => [
              ...prev,
              `Eval result: ${JSON.stringify(value)}`,
            ]);
          }
        })
        .catch(error => {
          ...
          setMessages(prev => [
            ...prev,
            `Eval error: ${error.message || 'Unknown error'}`,
          ]);
        });
    }
  };
  ...
  return (
    <div style={{ padding: 12 }}>
      ...
      <p style={{ marginTop: 8 }}>你好，这是 React DevTools 面板。2222</p>
